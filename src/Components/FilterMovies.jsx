...
function FilterMovies({ filteredMovies, setFilteredMovie, movies }) {
    ...
    const changeHandler = () => {
        ...
        if (selectedGenre == 'genre' && selectedRate == "rate", selectedYear == "year")
        ...
        if (selectedYear != 'year') {
            ...
            filter = filter.filter(movie => {
                if (parseInt(movie.year) >= yearInt && parseInt(movie.year) <= (yearInt + 100))
                    return movie
            })
        }
    ...
    return ( <
            ...
            <
            select className = 'col-lg-3 col-md-6 col-sm-6 py-3  px-5 border-radius-0-8-rem sec-bg-color text-color-white fs-1-8-rem '
...
year.map(y =>
    <
    option key = { y.id }
    value = { y.valueInt } > { y.value } < /option> 
)
}
...
className = 'col-1 text-color-white fs-2-0-rem filterBtn  col-sm-12' > filter < /button>
